...
public class OperatorSeatsAdapter extends BaseAdapter {
  ...
  public OperatorSeatsAdapter(boolean notLocal, Activity aty, List<OperatorSeat> operatorSeats) {
    this.aty = aty;
  ...
  public View getView(int position, View convertView, ViewGroup parent) {
    ...
    holder.total_seat.setText("Seats : " + (getItem(position).getPermitseatTotal() - getItem(
        position).getPermitseatSoldtotal()));

    NumberFormat nf = NumberFormat.getInstance();

    if (!notLocal) {
      if (getItem(position).getPrice() != null) {
        //Change (0,000,000) format_
    ...
    if (getItem(position).getExtraCity() != null) {
      if (getItem(position).getExtraCity().length() > 0) {
        ...
        holder.txt_fromto_city.setText(
            getItem(position).getFromName() + "=>" + getItem(position).getToName());
        holder.txt_extra_city.setText(
            aty.getResources().getString(R.string.strmm_boarding_place) + "" + getItem(
                position).getExtraCity());
      } else {
