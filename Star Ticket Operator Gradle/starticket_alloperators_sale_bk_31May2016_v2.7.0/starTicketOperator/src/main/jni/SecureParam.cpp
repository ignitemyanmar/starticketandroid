 ...
 */
 JNIEXPORT jstring JNICALL Java_com_ignite_mm_ticketing_application_SecureParam_deleteTicketParam
  (JNIEnv * env, jobject pObj, jstring token, jstring id, jstring date, jstring seat, jstring user, jstring rmarkType, jstring rmark){
	...
	const char *remarkType = env->GetStringUTFChars(rmarkType, JNI_FALSE);
	const char *remark = env->GetStringUTFChars(rmark, JNI_FALSE);

	stream << "{\"access_token\":\""<<access_token<<"\",\"trip_id\":\""<<trip_id<<"\",\"departure_date\":\""<<departure_date<<"\",\"seat_no\":\""<<seat_no<<"\",\"user_id\":\""<<user_id<<"\",\"remarkType\":\""<<remarkType<<"\",\"remark\":\""<<remark<<"\"}";
